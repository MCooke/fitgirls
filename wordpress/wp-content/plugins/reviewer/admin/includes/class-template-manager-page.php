	...
	public static function get_template_fields()
	{
		...
		return array(
			...
			'template_schema_type' => array(
				...
				'description'	=> sprintf(__( 'Define the type of Reviews Boxes for Google Rich Snippets. For more informations visit %s site', $plugin_slug ), '<a href="http://schema.org/" target="_blanck">Schema.org</a>'),
				'type'			=> 'text'
			),
 ...
 *----------------------------------------------------------------------------*/
	...
	{	
		...
		foreach ($options as $type => $type_data) {
			...
			echo '<li><input type="radio" id="rwp-radio-scheme-type-'. $type .'" name="'. $this->option_name .'[' . $field_id . ']" value="'. $type .'" '. $ck .' /><label title="" for="rwp-radio-scheme-type-'. $type .'">'. $type .'</label></li>';
		}
		echo '</ul>';
	}
