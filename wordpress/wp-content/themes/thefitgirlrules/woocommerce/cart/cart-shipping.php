...
<?php elseif ( ! WC()->customer->get_shipping_state() || ! WC()->customer->get_shipping_postcode() ) : ?>
	...
	<?php if ( is_cart() ) : ?>
		...
		<p><?php _e( 'No delivery methods were found; please recalculate your delivery or continue to checkout and enter your full address to see if there is delivery available to your location.', 'woocommerce' ); ?></p>
...
<?php else : ?>
	...
	<?php if ( is_cart() ) : ?>
		...
		<?php echo apply_filters( 'woocommerce_cart_no_shipping_available_html',
			...
			sprintf( __( 'Sorry, delivery is unavailable %s.', 'woocommerce' ) . ' ' . __( 'If you require assistance or wish to make alternate arrangements please contact us.', 'woocommerce' ), WC()->countries->shipping_to_prefix() . ' ' . WC()->countries->countries[ WC()->customer->get_shipping_country() ] ) .
			'</p></div>'
		); ?>
