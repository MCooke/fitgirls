...
class Lollum_Widget_Flickr extends WP_Widget {
	
	function Lollum_Widget_Flickr() {

		$widget_ops = array(
			'classname' => 'lol_widget_flickr',
			'description' => __('Use this widget to display your Flickr photos in your sidebar.', 'lollum')
		);
		$control_ops = array('id_base' => 'lol_widget_flickr');
		$this->WP_Widget('lol_widget_flickr', __('Custom Flickr Widget', 'lollum'), $widget_ops, $control_ops);
		...
		<div class="flickr-widget">
			...
			(function($){
				...
				function(data){$.each(data.items,
					function(i,item) {
						if(i < limit) {
							$(".flickr-widget").append("<a class='flickr-item-" + $i + "' href='" + item.media.m + "' title='" +  item.title +"'><img src='" + item.media.m + "'></a>");
							$i++;
						}
					});
				});
...
add_action('widgets_init', 'lol_flickr_widget');

function lol_flickr_widget() {
	register_widget('Lollum_Widget_Flickr');
}